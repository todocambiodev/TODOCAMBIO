...
async function enviarDatos(url) {
    ...
    let anuncioCopBuy = { precio: "" };
    let anuncioCopSell = { precio: "" };
        ...
        try {
            ...
            const procesarAnuncio = async (fiat, tipo, cache) => {
                ...
                if (lista.length > 1 && cache.precio !== lista[1].precio) {
                    ...
                    nuevoAnuncio.hoja = `${fiat.toUpperCase()} ${tipo.toUpperCase()}`;
                    
                    console.log(`Enviando: ${JSON.stringify(nuevoAnuncio)}`);
                    ...
                    console.log(`Respuesta GAS: ${resultado} Fecha: ${new Date()}`);
            ...
            anuncioCopBuy = await procesarAnuncio('cop', 'buy', anuncioCopBuy);
            anuncioCopSell = await procesarAnuncio('cop', 'sell', anuncioCopSell);
            anuncioUsdBuy = await procesarAnuncio('usd', 'buy', anuncioUsdBuy);
            anuncioUsdSell = await procesarAnuncio('usd', 'sell', anuncioUsdSell);
...
async function main() {
    try {
        ...
        await Promise.all([
            ...
            anunciosP2P("cop", "buy", ["Nequi"], montoCop, verificados, debug),
            anunciosP2P("cop", "sell", ["Nequi"], montoCop, verificados, debug),
            anunciosP2P("usd", "buy", ["Zelle"], montoUsd, verificados, debug),
            anunciosP2P("usd", "sell", ["Zelle"], montoUsd, verificados, debug),
            enviarDatos(url)
        ]);
